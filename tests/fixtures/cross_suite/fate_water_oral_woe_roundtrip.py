"""Deterministic Fate surface-water -> Exposure -> WoE round-trip fixture builder.

This fixture freezes an explicit concentration-to-intake bridge for
environmental surface-water context. It intentionally does not reuse the
...
transparent drinking-water assumptions.
"""
...
WOE_ROUNDTRIP_FIXTURE_PATH = (
    ...
    / "cross_suite"
    / "woe_ngra"
    / "fate_water_oral_exposure_handoff.v1.1.0.json"
)
WOE_SYNC_TARGET_PATH = (
    ...
    / "integration"
    / "__fixtures__"
    / "fate-water-oral-exposure-woe-roundtrip.bundle.json"
)

DETERMINISTIC_GENERATED_AT = "2026-04-21T14:00:00.000Z"
SOURCE_VERSION = "1.1.0"
SCHEMA_VERSION = "1.1.0"
BUNDLE_ID = "fate-water-oral-exposure-handoff-001"
CREATED_BY = "direct-use-exposure-fate-water-oral-fixture-builder"
PRODUCER_MODULE = "direct_use_exposure"

SCENARIO_DEFINITION_ID = "exp-fate-surface-water-definition-001"
ROUTE_DOSE_ARTIFACT_ID = "exp-fate-surface-water-route-dose-001"
SOURCE_FATE_EVIDENCE_ID = "fate-exp-water-001"
SOURCE_SURFACE_ARTIFACT_ID = "fate-surface-water-001"

BODY_WEIGHT_KG = 70.0
DRINKING_WATER_INTAKE_L_PER_DAY = 2.0
EVENTS_PER_DAY = 1.0
...
def _translation_summary(source_evidence: dict[str, Any]) -> dict[str, float | str]:
    concentration_mg_per_l = float(source_evidence["doseValue"])
    daily_water_intake_l = DRINKING_WATER_INTAKE_L_PER_DAY * EVENTS_PER_DAY
    external_mass_mg_per_day = concentration_mg_per_l * daily_water_intake_l
    external_dose_mg_per_kg_day = external_mass_mg_per_day / BODY_WEIGHT_KG
    return {
        "concentrationMgPerL": concentration_mg_per_l,
        "drinkingWaterIntakeLPerDay": DRINKING_WATER_INTAKE_L_PER_DAY,
        "eventsPerDay": EVENTS_PER_DAY,
        "ingestedWaterLPerDay": daily_water_intake_l,
...
def _build_fit_for_purpose() -> FitForPurpose:
    return FitForPurpose(
        label="bounded_environmental_oral_translation",
        suitable_for=[
            "Surface-water concentration to external-dose screening translation.",
            "Traceable Fate -> Exposure -> IVIVE/WoE integration testing.",
        ],
        not_suitable_for=[
            "Food-mediated dietary residue interpretation.",
            "Internal dose interpretation without downstream TK/PBPK review.",
            "Soil or crop-uptake environmental-media translation.",
        ],
    )
...
def _build_route_dose_provenance(summary: dict[str, float | str]) -> ProvenanceBundle:
    defaults_hash = _hash_value(
        {
            "body_weight_kg": BODY_WEIGHT_KG,
            "drinking_water_intake_l_per_day": DRINKING_WATER_INTAKE_L_PER_DAY,
            "events_per_day": EVENTS_PER_DAY,
        }
    )
    return ProvenanceBundle(
        algorithm_id="concentration_to_intake.drinking_water_screening.v1",
        plugin_id="fate_surface_water_concentration_to_intake_fixture",
        ...
        notes=[
            (
                "This deterministic fixture translates surface-water concentration into "
                "a bounded external oral dose using explicit screening assumptions."
            ),
            (
                f"Concentration {summary['concentrationMgPerL']:.15f} mg/L x "
                f"drinking-water intake {summary['drinkingWaterIntakeLPerDay']} "
                f"L/day / {BODY_WEIGHT_KG} kg."
            ),
        ],
    )
...
def _build_scenario_definition(
    ...
    return ExposureScenarioDefinition(
        ...
        notes=[
            (
                "Scenario definition is driven by surface-water concentration from Fate "
                "MCP rather than a product-use or food-residue regimen."
            ),
            (
                "The exposure layer acts here as an explicit concentration-to-intake "
                "consumer, not as a direct-use or dietary model."
            ),
        ],
    )
...
def _build_route_dose_estimate(
    *,
    ...
    return RouteDoseEstimate(
        ...
        limitations=[
            LimitationNote(
                code="surface_water_screening_translation",
                severity=Severity.WARNING,
                message=(
                    "Route-dose estimate uses explicit screening assumptions for "
                    "drinking-water intake, body weight, and daily intake frequency."
                ),
            ),
            LimitationNote(
                code="environmental_context_not_food_mediated",
                severity=Severity.WARNING,
                message=(
                    "Surface-water concentration remains environmental-media context and does not "
                    "represent food-mediated residue intake."
                ),
            ),
        ],
        qualityFlags=[
        ...
        notes=[
            ...
            (
                "Soil and crop-uptake environmental oral scenarios remain out of scope "
                "for this surface-water bridge fixture."
            ),
        ],
    )


def build_fate_water_oral_woe_roundtrip_bundle() -> dict[str, Any]:
    ...
    route_hint = (
        _find_identifier(source_evidence, "route_hint")
        or "water_contact_or_drinking_water_precursor"
    )
    ...
    evidence_item = {
        "originalId": "exp-fate-surface-water-001",
        "evidenceClass": "exposure",
        "sourceModule": "exposure_ingress_v1",
        "provenance": _provenance(
            tool_run_id="exp-fate-surface-water-001-run",
            source_hash_value={
        ...
        "methodDescription": (
            "Surface-water concentration-to-intake screening bridge derived from Fate governed "
            "handoff output with explicit drinking-water assumptions."
        ),
        "studyIdentifiers": [
            ...
            {"identifierType": "route_hint", "identifierValue": route_hint},
            {
                ...
                "identifierValue": "drinking_water_concentration_to_intake",
            },
        ...
        "route": "oral",
        "productCategory": "environmental_drinking_water_screening",
        ...
        "routeMetricKeys": [
            "drinking_water_intake_l_per_day",
            "events_per_day",
            "external_mass_mg_per_day",
            "surface_water_concentration_mg_per_l",
        ...
        "upstreamArtifactRefs": [
            _typed_ref(
                ...
                retrieval_endpoint="exposure-scenario://cross-suite/fate-water-oral/scenario-definition",
                cached_snapshot=scenario_definition_snapshot,
            ),
            _typed_ref(
                object_type_ref="RouteDoseEstimate",
                artifact_id=ROUTE_DOSE_ARTIFACT_ID,
                retrieval_endpoint="exposure-scenario://cross-suite/fate-water-oral/route-dose",
    ...
    claim_item = {
        "originalId": "exp-fate-surface-water-claim-001",
        "claimText": (
            "Surface-water concentration-to-intake bridge preserves bounded environmental "
        ...
        "lineOfEvidenceId": "loe-fate-water-oral-bridge",
        "rationale": (
            f"{source_claim['claimText']} The exposure bridge converts surface-water concentration "
            "into an external oral dose using explicit drinking-water assumptions and preserves "
            "the upstream concentration surface lineage."
        ),
        "provenance": _provenance(
            tool_run_id="exp-fate-surface-water-claim-001-run",
            source_hash_value={
                "evidenceObjectId": evidence_item["originalId"],
                "routeDoseValue": route_dose_estimate.dose.value,
            },
        ),
        "applicabilityRecordId": "exp-fate-surface-water-app-001",
    }

    applicability_item = {
        "originalId": "exp-fate-surface-water-app-001",
        "evidenceClass": "exposure",
        ...
        "dimensionAssessments": [
            {
                ...
                "rationale": (
                    ...
                    "surface-water concentration surface."
                ),
            ...
            },
            {
                ...
                "status": "direct",
                "rationale": (
                    "Surface-water route hint has been translated into an explicit oral "
                    "screening route."
                ),
                "evidenceValue": "oral",
                "targetValue": "oral",
            },
            {
                ...
                "status": "partial",
                "rationale": (
                    "The bridge still represents surface-water environmental context and should "
                    "not be mistaken for food-mediated residue intake."
                ),
                "bridgingRationale": source_applicability["dimensionAssessments"][1]["rationale"],
                "evidenceValue": "surface_water",
        ...
        "provenance": _provenance(
            tool_run_id="exp-fate-surface-water-app-001-run",
            source_hash_value={
    ...
    uncertainty_item = {
        "originalId": "exp-fate-surface-water-unc-001",
        "uncertaintyClass": "comparability_indirectness",
        "burdenLevel": "moderate",
        "affectedObjectIds": [evidence_item["originalId"]],
        "rationale": (
            "External dose depends on explicit screening assumptions for drinking-water intake "
            "and body weight, while the upstream surface-water concentration remains "
            "environmental-media context rather than food-mediated intake."
        ),
        "reducibility": "partially_reducible",
        "directionality": "bidirectional",
        "mitigationPath": (
            "Replace screening defaults with population-specific water-ingestion parameters or "
            "measured downstream intake data when available."
        ),
        "provenance": _provenance(
            tool_run_id="exp-fate-surface-water-unc-001-run",
            source_hash_value={
    ...
    link_item = {
        "originalId": "exp-fate-surface-water-link-001",
        "sourceId": evidence_item["originalId"],
        "sourceType": "evidence",
        "targetId": claim_item["originalId"],
        "targetType": "claim",
        "relationType": "supports",
        "rationale": "The translated oral screening dose directly supports the bridge claim.",
        "strength": "direct",
        "bidirectional": False,
        "provenance": _provenance(
            tool_run_id="exp-fate-surface-water-link-001-run",
            source_hash_value={
                "sourceId": evidence_item["originalId"],
                "targetId": claim_item["originalId"],
            },
...
def write_fate_water_oral_woe_roundtrip_bundle(
    path: Path = WOE_ROUNDTRIP_FIXTURE_PATH,
) -> dict[str, Any]:
    payload = build_fate_water_oral_woe_roundtrip_bundle()
