...
from tests.fixtures.cross_suite.fate_soil_oral_woe_roundtrip import (
    SOURCE_SURFACE_ARTIFACT_ID,
    WOE_ROUNDTRIP_FIXTURE_PATH,
    build_fate_soil_oral_woe_roundtrip_bundle,
)


def test_fate_soil_oral_woe_roundtrip_bundle_matches_checked_in_fixture() -> None:
    generated = build_fate_soil_oral_woe_roundtrip_bundle()
    checked_in = json.loads(WOE_ROUNDTRIP_FIXTURE_PATH.read_text(encoding="utf-8"))
    assert generated == checked_in


def test_fate_soil_oral_woe_roundtrip_bundle_preserves_concentration_to_intake_lineage() -> None:
    bundle = build_fate_soil_oral_woe_roundtrip_bundle()

    assert bundle["bundleId"] == "fate-soil-oral-exposure-handoff-001"
    ...
    assert "agricultural_soil_concentration_mg_per_kg" in evidence["routeMetricKeys"]
    assert "soil_ingestion_mg_per_day" in evidence["routeMetricKeys"]
    ...
    assert identifiers["route_mechanism"] == "soil_contact_concentration_to_intake"
