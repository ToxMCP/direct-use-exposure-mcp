...
def test_http_security_config_defaults_include_timeout_and_concurrency() -> None:
    ...
    assert config.origin_enforcement_enabled is True
    assert config.origin_allowlist_enabled is False
    assert "origin-validation(fail-closed)" in describe_http_boundary_security(config)


def test_streamable_http_app_allows_no_origin_without_allowlist() -> None:
...
def test_streamable_http_app_rejects_origin_without_allowlist() -> None:
    server = create_mcp_server()
    app = create_streamable_http_app(
        server,
        build_http_boundary_security_config(
...
def test_streamable_http_app_enforces_origin_allowlist() -> None:
    ...
    assert allowed_post.status_code != 403
    assert blocked_post.status_code == 403
    assert blocked_post.json()["error"] == "origin_not_allowed"
