...
def test_worker_dermal_execution_applies_explicit_formulation_profile_adjustment() -> None:
    aerosol_request = _base_request().model_copy(
        update={
            "product_use_profile": _base_request().product_use_profile.model_copy(
                update={
    ...
    assert any(
        flag.code == "worker_dermal_pressurized_aerosol_formulation_profile_adjustment_defaulted"
        for flag in result.quality_flags
    )
