...
def test_herbal_medicinal_valerian_direct_oral_executes_posology_check() -> None:
    engine = build_engine()
    request = ExposureScenarioRequest(
        chemical_id="HMPC_VALERIAN_2015",
        chemical_name="Benchmark Valerian Root Extract",
        route=Route.ORAL,
        scenario_class=ScenarioClass.SCREENING,
        product_use_profile=ProductUseProfile(
            product_category="herbal_medicinal_product",
            product_subtype="valerian_root_extract",
    ...
    assert "ema_valerian_root_oral_posology_2015" in (
        scenario.validation_summary.external_dataset_ids
    )
    checks = scenario.validation_summary.executed_validation_checks
    assert len(checks) == 1
    assert checks[0].check_id == "herbal_medicinal_valerian_oral_daily_mass_2015"
    assert checks[0].status.value == "pass"
    assert checks[0].observed_value == pytest.approx(1500.0, rel=1e-6)
...
def test_herbal_recovery_patch_executes_label_amount_check() -> None:
    engine = build_engine()
    request = ExposureScenarioRequest(
        chemical_id="ARNICA-PATCH-001",
        chemical_name="Benchmark Arnica Patch Constituent",
        route=Route.DERMAL,
        scenario_class=ScenarioClass.SCREENING,
        product_use_profile=ProductUseProfile(
            product_category="herbal_topical_product",
    ...
    assert "nlm_dailymed_activmend_patch_label_2025" in (
        scenario.validation_summary.external_dataset_ids
    )
    checks = scenario.validation_summary.executed_validation_checks
    assert len(checks) == 1
    assert checks[0].check_id == "herbal_recovery_patch_label_amount_2025"
    assert checks[0].status.value == "pass"
    assert checks[0].observed_value == pytest.approx(14.0, rel=1e-6)
    assert checks[0].reference_dataset_id == "nlm_dailymed_activmend_patch_label_2025"


def test_capsicum_hydrogel_patch_executes_label_amount_check() -> None:
    engine = build_engine()
    request = ExposureScenarioRequest(
        chemical_id="CAPSICUM-PATCH-001",
        chemical_name="Benchmark Capsicum Patch Constituent",
        route=Route.DERMAL,
        scenario_class=ScenarioClass.SCREENING,
        product_use_profile=ProductUseProfile(
    ...
    assert "nlm_dailymed_upup_capsicum_patch_label_2025" in (
        scenario.validation_summary.external_dataset_ids
    )
    checks = scenario.validation_summary.executed_validation_checks
    assert len(checks) == 1
    assert checks[0].check_id == "capsicum_hydrogel_patch_label_amount_2025"
    assert checks[0].status.value == "pass"
    assert checks[0].observed_value == pytest.approx(1.0, rel=1e-6)
    assert checks[0].reference_dataset_id == "nlm_dailymed_upup_capsicum_patch_label_2025"
...
def test_personal_care_deodorant_aerosol_uses_explicit_carrier_family_adjustment() -> None:
    request = InhalationScenarioRequest(
        chemical_id="DTXSID123",
        route=Route.INHALATION,
        product_use_profile=ProductUseProfile(
    ...
    assert constrained.external_dose.value < baseline.external_dose.value
    assert any(
        item.code == "pressurized_aerosol_carrier_family_adjustment_defaulted"
        for item in constrained.quality_flags
    )


def test_personal_care_deodorant_aerosol_uses_explicit_formulation_profile_adjustment() -> None:
    request = InhalationScenarioRequest(
        chemical_id="DTXSID123",
        route=Route.INHALATION,
        product_use_profile=ProductUseProfile(
    ...
    constrained = build_engine().build(request)
    assumptions = {item.name: item for item in constrained.assumptions}
    ...
    assert any(
        item.code == "pressurized_aerosol_formulation_profile_adjustment_defaulted"
        for item in constrained.quality_flags
    )
