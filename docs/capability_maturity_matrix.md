...
| --- | --- | --- | --- | --- |
| Core deterministic exposure engine | Dermal, oral, inhalation screening, aggregate summaries, scenario comparison, PBPK-ready export | `benchmark-regressed`, `curated`, `heuristic screening` | Benchmark corpus, defaults registry, executable reference bands | Some branches still depend on heuristic screening defaults, even though inhalation now has bounded saturation/deposition caps, bounded extrathoracic oral-handoff metrics, and family-, subtype-, carrier-, formulation-, and volatility/low-MW-aware bounded pressurized-aerosol mass interpretation, worker dermal now has retained-loading, breakthrough-lag, and evaporation-competition bounds, and herbal/TCM/supplement direct-use families now carry source-backed route-semantics anchors plus narrow EMA herbal-medicinal oral bands, an official supplement-label daily-mass band, executable topical-herbal geometry and delivered-amount anchors, and official topical patch unit-mass anchors rather than documentation-only routing notes. |
| Tier B/Tier C bounded uncertainty | Envelopes, parameter bounds, packaged probability-bounds profiles, scenario packages | `curated`, `heuristic screening` | Packaged archetype/profile registries with explicit manifests | Not a probabilistic population engine; no Monte Carlo claims, and mechanistic constraints are surfaced qualitatively through the uncertainty register rather than solved as full stochastic physics. |
| Residual-air and Tier 1 NF/FF inhalation | Anchored residual-air reentry, native treated-surface residual-air reentry, Tier 1 screening, packaged Tier 1 profiles | `benchmark-regressed`, `curated`, `heuristic screening` | External reference bands, sparse time-series packs, profile packs | Time-resolved validation is still sparse and product-family selective, native treated-surface reentry is a bounded first-order emission model, Tier 1 can apply a bounded local-entrainment floor for very weak static mixing, and deposition remains a bounded first-order sink rather than full aerosol dynamics. |
...
| Worker inhalation routing and execution | Worker routing, Tier 2 bridge, ART-style ingest, bounded execution, ART external exchange | `external-normalized`, `bounded surrogate`, `curated` | Worker determinant templates, external ART package exchange, worker reference bands | Not a native ART solver; external ART results remain imported, not internally solved, and task-intensity physiology plus LEV/control-context/capture-zone/capture-distance/capture-velocity effects remain bounded refinements rather than measured ventilation or capture-efficiency profiles, even when callers provide explicit LEV-family, hood-face-velocity, and measured-profile-band inputs. |
