...
| Worker inhalation routing and execution | Worker routing, Tier 2 bridge, ART-style ingest, bounded execution, ART external exchange | `external-normalized`, `bounded surrogate`, `curated` | Worker determinant templates, external ART package exchange, worker reference bands | Not a native ART solver; external ART results remain imported, not internally solved, and both task-intensity physiology and LEV/control-context effects remain bounded heuristic refinements rather than measured ventilation or capture-efficiency profiles. |
