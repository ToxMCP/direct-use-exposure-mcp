...
- Set `EXPOSURE_SCENARIO_MCP_HTTP_ALLOWED_ORIGINS` or pass `--http-allowed-origin` for any
  browser-based client. Requests without an `Origin` header are accepted for non-browser MCP
  clients; requests with an unconfigured or untrusted `Origin` are rejected.
...
- Long-running calculations (Tier B envelopes, Tier C probability bounds, integrated workflows)
  are bounded by the default in-process request timeout for `streamable-http`. Set upstream
  timeouts based on your latency requirements as a second boundary.
...
- bearer-token auth, fail-closed Origin validation, origin allow-list enforcement,
  request-size limits, request timeouts, concurrency limits, and append-only JSONL audit events
  for `streamable-http`
