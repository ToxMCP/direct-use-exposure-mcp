...
| `Worker dermal absorbed-dose execution` | Exports and ingests dermal absorbed-dose/PPE handoffs, then executes a bounded dermal kernel with retained-loading/runoff caps, barrier-material and chemistry modifiers, bounded breakthrough-lag timing, duration-aware evaporation competition, and category- plus subtype-aware pressurized-aerosol mass interpretation when only volumetric aerosol defaults are available, while keeping certified glove performance and full permeation modeling explicitly out of scope. |
