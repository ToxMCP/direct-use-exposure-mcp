...
def tier1_inhalation_parameter_guide() -> str:
    ...
    lines.extend(
        [
            ...
            "## Two-Zone Solver (v1)",
            "",
            "A deterministic mass-conserving two-zone model is now available alongside the legacy",
            "heuristic path.  It solves coupled near-field (NF) and far-field (FF) ODEs with an",
            "exact analytical primary path.",
            "",
            "### Solver selection",
            "",
            "- `solver_variant=auto` (default) routes to the legacy heuristic while benchmark",
            "  migration is in progress.",
            "- `solver_variant=heuristic_v1` forces the legacy heuristic path.",
            "- `solver_variant=two_zone_v1` forces the new two-zone mass-balance path.",
            "",
            "### Retained parameters",
            "",
            "- `airflow_directionality` -> drives the default interzonal exchange turnover.",
            "- `near_field_volume_m3` and `room_volume_m3` -> define NF and FF control volumes.",
            "- `spray_duration_seconds` and `use_amount_per_event` -> drive the emission rate.",
            "- `particle_size_regime` -> informs deposition defaults (not a direct concentration",
            "  multiplier in the two-zone path).",
            "- `aerosolized_fraction` -> mass fraction released to air.",
            "",
            "### Deprecated as direct concentration multipliers",
            "",
            "- `distance_factor` — removed from the concentration equation; kept only for profile",
            "  alignment diagnostics.",
            "- `particle_persistence_factor` — deprecated as a direct NF multiplier; particle",
            "  effects are now expressed through deposition and loss defaults.",
            "",
            "### New optional expert overrides",
            "",
            "- `interzonal_flow_rate_m3_per_hour` (beta)",
            "- `near_field_loss_rate_per_hour` (k_NF)",
            "- `far_field_loss_rate_per_hour` (k_FF)",
            "- `source_allocation_to_near_field_fraction` (eta)",
            "- `ventilation_allocation_to_near_field_fraction` (phi)",
            "",
            "### Default physics",
            "",
            "- Source split to NF: eta = 1.0 (unless profile or override says otherwise).",
            "- Ventilation split to NF: phi = 0.0 (all exhaust assigned to FF by default).",
            "- Interzonal flow: beta = max(V_NF * exchange_turnover, local_entrainment_floor).",
            "  The legacy addition of `air_exchange_rate_per_hour` to this product has been",
            "  removed to avoid double-counting room ventilation.",
            "- Zone losses: k_NF = k_FF = deposition_rate_per_hour (Phase-1 default).",
            "",
            "## Usage Notes",
            "",
...
def release_notes_markdown(report: ReleaseMetadataReport) -> str:
    lines = [
        ...
        "",
        f"Direct-Use Exposure MCP `v{report.release_version}` is a hardening patch for the "
        "auditable deterministic exposure-screening MCP surface. This cut tightens "
        "streamable-http Origin handling, refreshes release-artifact verification, and keeps "
        "the public external-dose surface ready for regulatory-facing review workflows.",
        "",
        "## Public Positioning",
        "",
