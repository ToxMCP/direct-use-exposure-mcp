...
ASSUMPTION_DOMAIN_FIELDS: dict[str, tuple[str, ...]] = {
    ...
    "room_volume_m3": ("region", "product_category", "product_subtype", "application_method"),
    "air_exchange_rate_per_hour": (
        "region",
        "product_category",
        "product_subtype",
    ...
    "concentration_fraction": ("product_category", "product_subtype", "physical_form"),
}
...
class AssumptionTracker:
    ...
    def _evidence_grade(
        ...
        if source_kind != SourceKind.DEFAULT_REGISTRY:
            return None
        if is_route_semantic_source_id(source.source_id):
