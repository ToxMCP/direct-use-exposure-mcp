...
* SCIENTIFIC codes emitted by the vendored spine policy engine itself
  (e.g. ``EXTERNAL_EXPOSURE_NOT_INTERNAL_DOSE``,
  ``EXPOSURE_UNCERTAINTY_AND_CEILING_REQUIRED``,
  ``EXPOSURE_SCENARIO_CONTEXT_REQUIRED``). These are passed through verbatim
  from the engine's ``failures[]``.
...
#: serializer) — validated BEFORE any projection. Blocks; the packet is never
#: projected. This guard closes the producer-emission-contract dead-arm class: a
#: "fault" that can only fire a scientific code by carrying a schema-forbidden /
#: undeclared field is caught here as a contract violation instead of silently
#: advertising a code that never bites on a real producer-emitted packet. (Code
#: constant lives in source_contract.py to avoid an import cycle; re-exported into
#: META_FAIL_CLOSED_CODES below.)
