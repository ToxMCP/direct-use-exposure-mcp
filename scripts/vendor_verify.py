...
def _vendored_files() -> list[Path]:
    ...
    return sorted(p for p in VENDOR_ROOT.rglob("*") if p.is_file() and p.name != MANIFEST_NAME)
...
def write_manifest() -> int:
    ...
    manifest = {
        ...
        "note": (
            ...
            "engine for the direct-use-exposure-mcp Track-B scientific-invariants gate. "
...
def verify() -> int:
    ...
    for rel, expected in sorted(recorded.items()):
        ...
        actual = _sha256(path)
        if actual != expected:
            problems.append(f"DIGEST MISMATCH {rel}: expected {expected[:12]}… got {actual[:12]}…")
